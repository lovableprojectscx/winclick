...
const testimonials = [
  { name: "Lucía Ramírez", rank: "VIP", initials: "LR", text: "En 3 meses ya tengo 8 referidos directos y mis comisiones crecen cada semana. Winner Organa cambió mi vida.", earning: "S/ 1,200" },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        {/* Decorative orbs */}
        <div className="absolute top-20 right-20 w-[400px] h-[400px] rounded-full opacity-30" style={{ background: "radial-gradient(circle, rgba(242,201,76,0.12) 0%, transparent 70%)" }} />
        <div className="absolute bottom-20 left-10 w-[350px] h-[350px] rounded-full opacity-30" style={{ background: "radial-gradient(circle, rgba(46,204,113,0.08) 0%, transparent 70%)" }} />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16 relative z-10 w-full">
          {/* Pill badge */}
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-wo-pill mb-8" style={{ background: "rgba(242,201,76,0.1)", border: "0.5px solid rgba(242,201,76,0.25)" }}>
            <span className="w-2 h-2 rounded-full bg-secondary animate-pulse" />
            <span className="font-jakarta text-xs font-medium text-primary">Oportunidad de negocio real</span>
          </div>

          <h1 className="font-syne font-extrabold text-[52px] md:text-[72px] leading-[1.1] tracking-[-0.01em] mb-6">
          ...
          <p className="font-jakarta text-[15px] text-wo-crema-muted max-w-[380px] leading-[1.65] mb-8">
            Únete a la comunidad de socios que generan ingresos vendiendo productos orgánicos premium y construyendo su propia red.
          </p>

          <div className="flex flex-wrap gap-3 mb-12">
            <Link to="/registro-afiliado" className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-jakarta font-bold text-sm px-6 py-3 rounded-wo-btn hover:bg-wo-oro-dark transition-colors">
          ...
          {/* Stats */}
          <div className="flex items-center gap-0 pt-8" style={{ borderTop: "0.5px solid rgba(255,255,255,0.07)" }}>
            {[
              { num: "+2,400", label: "Socios activos" },
              { num: "25%", label: "Comisión máxima" },
              { num: "10", label: "Niveles de red" },
            ].map((s, i) => (
              <div key={i} className={`flex-1 ${i > 0 ? "pl-6 md:pl-8" : ""}`} style={i > 0 ? { borderLeft: "0.5px solid rgba(255,255,255,0.07)" } : {}}>
                <p className="font-syne font-extrabold text-[28px] md:text-[30px] text-primary">{s.num}</p>
      ...
      <section className="bg-wo-grafito py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="font-jakarta text-[10px] font-bold tracking-[0.16em] uppercase text-primary mb-3">POR QUÉ WINNER ORGANA</p>
                ...
                <div key={cl.level} className="flex items-center gap-3 group">
                  ...
                  <div className="flex-1 h-8 rounded-md overflow-hidden bg-wo-carbon relative">
                    <div
                      ...
                      style={{ width: `${barWidth}%`, background: isSpike ? "hsl(var(--wo-oro))" : cl.level <= 3 ? "rgba(242,201,76,0.85)" : cl.level <= 7 ? "rgba(242,201,76,0.55)" : "rgba(242,201,76,0.35)" }}
                    >
      ...
      <section className="bg-background py-24 relative overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-[500px] h-[500px] rounded-full" style={{ background: "radial-gradient(circle, rgba(242,201,76,0.08) 0%, transparent 60%)" }} />
        </div>
        <div className="max-w-lg mx-auto px-4 text-center relative z-10">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-wo-pill bg-secondary/10 text-secondary font-jakarta text-xs font-bold mb-6" style={{ border: "0.5px solid rgba(46,204,113,0.25)" }}>
          ...
          <Link to="/registro-afiliado" className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-jakarta font-bold text-sm px-9 py-4 rounded-xl hover:bg-wo-oro-dark transition-colors">
